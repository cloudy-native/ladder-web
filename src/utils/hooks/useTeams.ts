    ...
    try {
      ...
      if (teamData && Array.isArray(teamData)) {
        const validTeams = teamData.filter(
          (team) => team !== null && typeof team === "object" && team.id && team.name
        );
        ...
        const teamsWithPlayersPromises = validTeams.map(async (team) => {
          try {
            ...
            let ladder = null;
            if (team.ladderId) {
              const ladderResult = await client.models.Ladder.get({
                id: team.ladderId,
              });
...
export function useTeamCreate() {
  ...
  const createTeam = useCallback(async (name: string, rating: string = "1200", player1Id?: string) => {
    // Reset error state
    setCreateError(null);
...
export function useTeamDelete() {
  ...
  const deleteTeam = useCallback(async (id: string, teams: TeamWithPlayers[]) => {
...
export function useTeamJoin() {
  ...
  const joinTeam = useCallback(async (teamId: string, currentPlayer: Player, teams: TeamWithPlayers[]) => {
    // Reset error state
    setJoinError(null);
    ...
    try {
      ...
      const playerAsPlayer1Teams = await client.models.Team.list({
        filter: { player1Id: { eq: currentPlayer.id } },
      });

      // Then check if current player is already on a team as player2
      const playerAsPlayer2Teams = await client.models.Team.list({
      ...
      if (isOnTeam) {
        ...
        if (playerAsPlayer1Teams.data && playerAsPlayer1Teams.data.length > 0) {
        ...
        if (currentTeamId && currentSlot) {
          ...
          const { errors: leaveErrors } = await client.models.Team.update({
            id: currentTeamId,
            [currentSlot]: null,
          });
  ...
  const leaveTeam = useCallback(async (teamId: string, currentPlayer: Player) => {
    setJoinError(null);
...
export function useTeamLadder() {
  ...
  const addTeamToLadder = useCallback(async (teamId: string, ladderId: string) => {
    setUpdateError(null);
    setIsUpdating(true);
...
export function usePlayer() {
  ...
  const fetchCurrentPlayer = useCallback(async () => {
    ...
    try {
      ...
      setError("Failed to get current player");
  ...
  const isPlayerInTeam = useCallback((teamId: string, teams: TeamWithPlayers[]) => {
    ...
    return (
      team.player1Id === currentPlayer.id || team.player2Id === currentPlayer.id
    );
  }, [currentPlayer]);

  const isPlayerOnAnyTeam = useCallback((teams: TeamWithPlayers[]) => {
    if (!currentPlayer) return false;
  ...
  return {
    ...
    isPlayerInTeam,
    isPlayerOnAnyTeam,
    refreshPlayer: fetchCurrentPlayer
  };
}
