...
const LADDER_DIVISIONS = [
  { name: "Pro Tour Elite", description: "The pinnacle of competitive racket sports - where champions are crowned" },
  { name: "Advanced Division", description: "High-level play for experienced competitors with refined technique" },
  { name: "Intermediate Power", description: "Solid players who've mastered the fundamentals of the game" },
  { name: "Challenger Circuit", description: "Developing players looking to test their skills against tougher competition" },
  { name: "Foundation League", description: "Building consistent play with an emphasis on technique development" },
  { name: "Recreational Plus", description: "Regular players with a competitive edge seeking organized matches" },
  { name: "Social Starters", description: "Introduction to competitive play in a friendly, supportive environment" },
];

// Clever/funny racket sports team names
const TEAM_NAMES = [
  // Racket sports puns - general
  "Net Profits", "Strings Attached", "Court Jesters", "Smash Hit",
  "Deuce & Associates", "The Racket-eers", "Serve-ivors", "Lob City",
  "Top Spinners", "Return Policy", "Just the Tip", "No Strings Attached",
  "Baseline Bandits", "Volley Llamas", "The Drop Shots", "Alley-Oops",
  "Slice & Dice", "Grip It & Rip It", "The Fault Finders", "Sweet Spot",
  "Tenacious Slice", "The Overhead Smashers", "Double Fault Coffee", "Hit & Giggle",
  "The Spin Doctors", "BackSpin Bandits", "Topspin Tyrants", "Rally Monkeys",
  
  // Tennis references
  "Grand Slammers", "Advantage Point", "Love-Forty Thieves", "The Break Points",
  "Deuce Bigalow", "New Balls Please", "Game, Set, Match", "You Got Served",
  "Federer Express", "Nadal's Biceps", "The Djokesters", "Serena's Sirens",
  "Wimble-Done", "Court of Appeals", "The Ballboys", "Hawkeye High Five",
  
  // Pickleball references
  "Dill With It", "The Pickleballers", "Kosher Dills", "Sweet Pickles",
  "Dinking Donuts", "Kitchen Sink", "Zero-Zero-Twos", "Picklenauts",
  "Gherkin Jerks", "In-A-Pickle", "Dinkable Feast", "No Dinking Way",
  
  // Squash references
  "Squash Goals", "Squash the Competition", "Wall Bangers", "Boast Masters",
  "Corner Pocket", "The Tin Men", "Nick Pick", "Squash Buckling Heroes",
  
  // Racquetball references
  "Blue Balls", "Kill Shot Crew", "Ceiling Service", "Wall Stars",
  "Off-The-Wall", "Drive-By Shooters", "Racquet Scientists", "Ball Busters",
  
  // Badminton references
  "Shuttle Runners", "Shuttlecocks", "Feather Friends", "The Clear Winners",
  "Smash Bros", "Birds of Play", "Shuttleworth It", "Cock A Hoop"
];
...
async function clearExistingData() {
  ...
  try {
    ...
    // Delete ladders
    const { data: ladders } = await client.models.Ladder.list();
    if (ladders && ladders.length > 0) {
      for (const ladder of ladders) {
        await client.models.Ladder.delete({ id: ladder.id });
      }
  ...
  try {
    for (let i = 0; i < count; i++) {
      ...
      if (createStar) {
        ...
        const email = faker.internet.email({ firstName: givenName, lastName: familyName }).toLowerCase();
...
async function generateLadders(count: number): Promise<Ladder[]> {
  ...
  try {
    for (let i = 0; i < actualCount; i++) {
      ...
      const { data, errors } = await client.models.Ladder.create({
        name: division.name,
  ...
  try {
    ...
    for (const ladderId in teamsByLadder) {
      ...
      for (let i = 0; i < ladderMatchCount; i++) {
        ...
        const createdAt = dates[Math.floor(Math.random() * dates.length)].toISOString();
        ...
        if (data) {
          ...
          if ((i + 1) % 5 === 0 || i === ladderMatchCount - 1) {
            console.log(`Created ${i + 1}/${ladderMatchCount} matches for ladder ${ladderId}`);
          }
        }
      }
    }
...
async function generateTeams(
  ...
  try {
    for (let i = 0; i < Math.min(count, shuffledTeamNames.length); i++) {
      ...
      const rating = baseRating + Math.floor((Math.random() + Math.random() + Math.random() - 1.5) * deviation);
      
      // Assign a racket sport type to this team
      const sportType = RACKET_SPORTS[Math.floor(Math.random() * RACKET_SPORTS.length)];
      ...
      if (data) {
        ...
        const { data: updatedTeam, errors: updateErrors } = await client.models.Team.update({
          id: data.id,
        ...
        if (updateErrors) {
          console.error(`Error updating team ${data.name} with players:`, updateErrors);
        }
        
        if (updatedTeam) {
          teams.push(updatedTeam);
          const ladderName = ladderId ? ladders.find(l => l.id === ladderId)?.name : "None";
          ...
          console.log(`Created ${sportType} team: ${updatedTeam.name} - Ladder: ${ladderName} - Players: ${playerInfo}`);
        }
      }
    }
