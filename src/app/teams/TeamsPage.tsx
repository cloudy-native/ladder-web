...
import { CreateTeamDialog, TeamCard } from "@/components/teams";
import { PAGE_SIZE } from "@/utils/constants";
import { TeamWithPlayers } from "@/utils/crudl";
import { formatPlayerName } from "@/utils/data";
import { useFilter, usePagination, useTeamList } from "@/utils/hooks";
import {
  Alert,
  Box,
  Button,
...
export function TeamsPage() {
  ...
  const teamFilter = useCallback(
    (team: TeamWithPlayers, searchText: string) => {
      // Check team name
      if (team.team.name.toLowerCase().includes(searchText.toLowerCase()))
        return true;

      // Check player names
      if (
        team.player1 &&
        formatPlayerName(team.player1).includes(searchText.toLowerCase())
      )
        return true;
      if (
        team.player2 &&
        formatPlayerName(team.player2).includes(searchText.toLowerCase())
      )
        return true;
